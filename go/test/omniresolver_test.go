...
import (
	...
	omni "github.com/voxgig-sdk/chocomilk-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.ChocomilkError); is {
				out := map[string]any{
					"name":    "ChocomilkError",
...
func omniCtx(arg any, client *sdk.ChocomilkSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.ChocomilkSDK); is {
			client = live
