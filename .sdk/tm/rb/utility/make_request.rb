...
module ChocomilkUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = ChocomilkResponse.new({})
    ...
    if fetch_err
      ...
      response = ChocomilkResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = ChocomilkResponse.new(fetched)
    else
