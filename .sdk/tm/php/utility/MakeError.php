...
// Chocomilk SDK utility: make_error

require_once __DIR__ . '/../core/Operation.php';
require_once __DIR__ . '/../core/Result.php';
require_once __DIR__ . '/../core/Error.php';

class ChocomilkMakeError
{
    public static function call(?ChocomilkContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new ChocomilkContext([], null);
        }
        $op = $ctx->op ?? new ChocomilkOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new ChocomilkResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof ChocomilkError) ? $err->msg : (string)$err;
        $msg = "ChocomilkSDK: {$opname}: {$errmsg}";
        ...
        $sdk_err = new ChocomilkError('', $msg, $ctx);
        ...
        if ($err instanceof ChocomilkError) {
