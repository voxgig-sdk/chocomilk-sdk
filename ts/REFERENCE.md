# Chocomilk TypeScript SDK Reference

Complete API reference for the Chocomilk TypeScript SDK.
...
```ts
new ChocomilkSDK(options?: object)
```
...
#### `ChocomilkSDK.test(testopts?, sdkopts?)`
...
**Returns:** `ChocomilkSDK` instance in test mode.
...
Create a new `YouTube` entity instance.
...
**Returns:** `YouTubeEntity` instance.
...
Return the parent `ChocomilkSDK` instance.
...
```ts
const results = await client.YouTube().list({ query: "example" })
```
