...
const TEST_SEED = { "entity": { "search": { "test01": { "id": "test01" } }, "you_tube": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+ChocomilkSDK\b/.test(code) ||
        /\bChocomilkSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+ChocomilkSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/ChocomilkSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
