...
class ChocomilkTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            ChocomilkTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = ChocomilkTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = ChocomilkTestRunner.getenv("CHOCOMILK_TEST_LIVE")
        override = ChocomilkTestRunner.getenv("CHOCOMILK_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = ChocomilkTestRunner.getenv(key)
        ...
        explain = ChocomilkTestRunner.getenv("CHOCOMILK_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["CHOCOMILK_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if ChocomilkTestRunner._test_control is not None:
            return ChocomilkTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                ChocomilkTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            ChocomilkTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = ChocomilkTestRunner.load_test_control()
...
def load_env_local():
    ChocomilkTestRunner.load_env_local()


def env_override(m):
    return ChocomilkTestRunner.env_override(m)


def entity_data(v):
    return ChocomilkTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return ChocomilkTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return ChocomilkTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return ChocomilkTestRunner.load_test_control()


def live_client_options():
    return ChocomilkTestRunner.live_client_options()


def live_delay_ms():
    return ChocomilkTestRunner.live_delay_ms()
