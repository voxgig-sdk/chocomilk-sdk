...
from chocomilk_sdk.chocomilk_types import (
    YouTube,
    YouTubeListMatch,
)
...
class YouTubeEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return YouTubeEntity(self._client, opts)
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[YouTube]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.YouTube().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
