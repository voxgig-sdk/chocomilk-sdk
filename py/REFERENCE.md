# Chocomilk Python SDK Reference

Complete API reference for the Chocomilk Python SDK.
...
#### `ChocomilkSDK.test(testopts=None, sdkopts=None)`
...
Create a new `YouTubeEntity` instance. Pass `None` for no initial data.
...
```python
results = client.YouTube().list({"query": "example"})
for you_tube in results:
    print(you_tube)
```
...
Create a new `YouTubeEntity` instance with the same options.
