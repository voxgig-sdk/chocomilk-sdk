...
[MCP](https://modelcontextprotocol.io) server exposing the Chocomilk SDK as
two agent tools — `chocomilk_list` and `chocomilk_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/chocomilk-mcp)
make build

# 2. Provide credentials via the environment
export CHOCOMILK_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user chocomilk \
  -- /absolute/path/to/chocomilk-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./chocomilk-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// chocomilk_list: first page of records
{ "entity": "search" }
{ "entity": "search", "query": { } }

// chocomilk_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/chocomilk-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user chocomilk \
     -- "$PWD"/dist/*/chocomilk-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `chocomilk_list` and `chocomilk_load` tools now appear
   in new sessions. Ask the agent to *"list search using chocomilk"*
   and it calls `chocomilk_list` with `{"entity":"search"}`.
...
```sh
export CHOCOMILK_APIKEY=sk_live_xxx            # API key
export CHOCOMILK_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `chocomilk_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `chocomilk_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `CHOCOMILK_APIKEY` | API key sent with every request. |
| `CHOCOMILK_BASE` | Optional override of the API base URL. |
...
```sh
./chocomilk-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"chocomilk_load","arguments":{"entity":"search","query":{"id":1}}}}'
```
