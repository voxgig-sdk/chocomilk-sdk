...
local YouTubeEntity = {}
YouTubeEntity.__index = YouTubeEntity


function YouTubeEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, YouTubeEntity)
  self._name = "you_tube"
  self._client = client
...
function YouTubeEntity:get_name()
  return self._name
end


function YouTubeEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return YouTubeEntity.new(self._client, opts)
end
...
function YouTubeEntity:mark_deleted()
  self._deleted = true
end


function YouTubeEntity:deleted()
  return true == self._deleted
end


function YouTubeEntity:data_set(args)
  if args ~= nil then
...
function YouTubeEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function YouTubeEntity:match_set(args)
  if args ~= nil then
...
function YouTubeEntity:match_get()
...
function YouTubeEntity:stream(action, args, callopts)
...
---@param reqmatch YouTubeListMatch
---@param ctrl? table
---@return YouTube[]
---@return string? err
function YouTubeEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function YouTubeEntity:_run_op(ctx, post_done)
  local utility = self._utility
