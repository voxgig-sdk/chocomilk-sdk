...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = ChocomilkOmni.make_runner(TEST_JSON_FILE, ChocomilkSDK.test(nil, nil))
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : ChocomilkOmni.errify(err)
    end
