...
module ChocomilkOmni
  ...
  class ObjView < ::Hash
    ...
    def view_wrap(name)
      ...
      return val if ChocomilkOmni.plainval?(val) || seen.key?(val.object_id)
    ...
    def view_attrname(key)
      ...
      snake = ChocomilkOmni.underscore(name)
    ...
    def keys
      out = view_names.map { |n| ChocomilkOmni.camelize(n) }
    ...
    def []=(key, val)
      ...
      t = @view_target
      name = view_attrname(key) || ChocomilkOmni.underscore(key.to_s)
      setter = "#{name}="
  ...
  def enrich(ctxmap, ctx)
    ...
    ctx.spec = ChocomilkSpec.new(spec_map) if spec_map.is_a?(::Hash)
    ...
    if res_map.is_a?(::Hash)
      ...
      if err_map.is_a?(::Hash) && err_map['message'].is_a?(::String) &&
        ...
        ctx.result.err = ChocomilkError.new('', err_map['message'])
      end
    end
  ...
  def sdkhooks(sdk)
    ...
    errify = ->(err) { ChocomilkOmni.errify(err) }
