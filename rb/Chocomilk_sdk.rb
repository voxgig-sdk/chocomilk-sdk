...
require_relative 'Chocomilk_types'
...
class ChocomilkSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = ChocomilkHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = ChocomilkHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, ChocomilkFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    ChocomilkUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = ChocomilkHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = ChocomilkHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = ChocomilkHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = ChocomilkHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = ChocomilkError.new(
        "graphql_error", "ChocomilkSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  # Canonical facade: client.YouTube.list / client.YouTube.load({ "id" => ... })
  def YouTube(data = nil)
    require_relative 'entity/you_tube_entity'
    YouTubeEntity.new(self, data)
  end
