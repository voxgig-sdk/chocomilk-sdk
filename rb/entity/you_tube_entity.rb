...
class YouTubeEntity
  ...
  def make
    opts = @_entopts.dup
    YouTubeEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = ChocomilkHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [YouTube, Hash] the current YouTube data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = ChocomilkHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of YouTube fields)
  def match_get
  ...
  #
  # @param reqmatch [YouTubeListMatch, Hash, nil] match filter (any subset of
  #   YouTube fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<YouTube>, Array] the matching YouTube items; raises ChocomilkError on failure
