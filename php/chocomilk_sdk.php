...
class ChocomilkSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = ChocomilkHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = ChocomilkHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!ChocomilkFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, ChocomilkFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return ChocomilkUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = ChocomilkHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = ChocomilkHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = ChocomilkHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new ChocomilkError($op . "_allow",
                "ChocomilkSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = ChocomilkHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new ChocomilkError("graphql_error",
                "ChocomilkSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    private $_you_tube = null;

    // Canonical facade: $client->YouTube()->list() / ->load(["id" => ...]).
    // PHP method names are case-insensitive, so lowercase $client->you_tube()
    // resolves here too.
    public function YouTube($data = null)
    {
        require_once __DIR__ . '/entity/you_tube_entity.php';
        if ($data === null) {
            if ($this->_you_tube === null) {
                $this->_you_tube = new YouTubeEntity($this, null);
            }
            return $this->_you_tube;
        }
        return new YouTubeEntity($this, $data);
    }
