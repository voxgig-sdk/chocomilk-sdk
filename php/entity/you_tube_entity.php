...
class YouTubeEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new YouTubeEntity($this->_client, $opts);
    }

    /**
     * @param YouTube|array $args YouTube data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = ChocomilkHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return YouTube|array The current YouTube data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of YouTube fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = ChocomilkHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of YouTube fields).
     */
    ...
    /**
     * List YouTube items matching the given filter.
     *
     * @param YouTubeListMatch|array|null $reqmatch Match filter (any subset
     *   of YouTube fields) as an assoc-array; YouTubeListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return YouTube[]|array A list of YouTube items as assoc-arrays at
     *   the SDK boundary; throws ChocomilkError on failure (item-5 convention).
     */
