...
class FtCtrl extends ChocomilkControl
{
...
class FtClient
{
    ...
    private ChocomilkUtility $utility;

    public function __construct(ChocomilkUtility $utility, array $options)
    {
    ...
    public function get_utility(): ChocomilkUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (ChocomilkContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public ChocomilkContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (ChocomilkContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = ChocomilkConfig::make_feature($name);
    ...
    private static function buildUrl(ChocomilkSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof ChocomilkError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new ChocomilkResponse($fetched);
        }
    ...
    private function fail(ChocomilkContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (ChocomilkContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
