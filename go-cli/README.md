...
boru-driven command-line client **and** interactive REPL for the Chocomilk
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/chocomilk-cli)
make build
...
./chocomilk-cli list search
./chocomilk-cli list you_tube

# 5. Override the API base URL for a single call
CHOCOMILK_BASE=https://api.example.com ./chocomilk-cli list search

# 6. No arguments -> interactive REPL
./chocomilk-cli
chocomilk> list search
chocomilk> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/chocomilk-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/chocomilk-cli list search
   ```
...
Run with no arguments to open a REPL (prompt `chocomilk>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/chocomilk-cli`. |
...
(`./chocomilk-cli <expr>`) and interactively (the REPL), and expressions compose the
