// Command Chocomilk-cli is a CLI + REPL for the Chocomilk SDK,
...
const prompt = "chocomilk"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "search you_tube"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      CHOCOMILK_APIKEY (api key), CHOCOMILK_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: CHOCOMILK_APIKEY carries the API key and
	// CHOCOMILK_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("CHOCOMILK_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("CHOCOMILK_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewChocomilkSDK(opts)
